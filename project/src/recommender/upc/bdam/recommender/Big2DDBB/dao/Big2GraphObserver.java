...
import upc.bdam.recommender.documentDDBB.dao.ReplicaDataAccessManager;
 ...
 *
 */
public class Big2GraphObserver implements GraphDDBBObserver {
	...
	public void insert(IBinding[] values) {
		try {
			if (values[0] instanceof Author)
				ReplicaDataAccessManager.getInstance().insert(Big2AccessManager.DOCUMENT_PERSON_INSERT,
						values);
			else if (values[0] instanceof Band)
				ReplicaDataAccessManager.getInstance().insert(Big2AccessManager.DOCUMENT_BAND_INSERT, values);
			else if (values[0] instanceof Genre)
				ReplicaDataAccessManager.getInstance().insert(Big2AccessManager.DOCUMENT_GENRE_INSERT, values);
			else
				ReplicaDataAccessManager.getInstance().insert(Big2AccessManager.DOCUMENT_ARTWORK_INSERT,
						values);
